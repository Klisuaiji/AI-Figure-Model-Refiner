"""V3: Hair via Voxel Remesh for true unified mass.

Yelzkizi approach: "convert multiple strands to a single mesh
or apply a uniform flat material to treat hair as one cohesive shape".

Steps:
1. Decimate to remove spiky outliers
2. VOXEL REMESH to get clean unified quad-based topology
3. Solidify 1.5mm for 3D print wall
4. Toon material with hard cel-shading
5. Smooth the result for anime curves
"""
import bpy
import bmesh
...
# ------------------------------------------------------------------
# STEP 2: VOXEL REMESH — convert to clean unified quad mesh
# ------------------------------------------------------------------
print("\n=== STEP 2: Voxel Remesh (unified topology) ===")
bpy.context.view_layer.objects.active = hair
hair.select_set(True)
# Voxel remesh in 5.2 is bpy.ops.voxel_remesh
# Use the operator directly
try:
    bpy.ops.object.voxel_remesh(
        voxel_size=0.015,  # 1.5cm voxels — coarse, unified look
        adaptivity=0.0,
        use_smooth_shade=True,
        use_remove_dissolved=True,
    )
    print(f"  Voxel remesh done: {len(hair.data.vertices)} verts, dims={hair.dimensions}")
except Exception as e:
    print(f"  voxel_remesh failed: {e}")
    # Try older API
    try:
        bpy.ops.object.modifier_add(type='REMESH')
        mod_rem = hair.modifiers[-1]
        mod_rem.mode = 'VOXEL'
        mod_rem.voxel_size = 0.005  # 5mm voxels — fine detail but still unified
        bpy.ops.object.modifier_apply(modifier=mod_rem.name)
        print(f"  Remesh applied: {len(hair.data.vertices)} verts, dims={hair.dimensions}")
    except Exception as e2:
        print(f"  Remesh modifier also failed: {e2}")
hair.select_set(False)

# ------------------------------------------------------------------
# STEP 3: Light Laplacian to soften voxel edges (2 iterations)
# ------------------------------------------------------------------
print("\n=== STEP 3: Light smoothing (2 iterations factor 0.12) ===")
for i in range(2):
    bm = bmesh.new()
    try:
...
# ------------------------------------------------------------------
# STEP 5: Toon material with HARD cel-shading
# ------------------------------------------------------------------
...
# Diffuse only (no specular for pure toon look)
...
print("  Toon material: 2-step cel-shading (dark + light)")

# Save
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT_DIR, "hair_anime_v3.blend"))
print(f"\nSaved hair_anime_v3.blend")
...
for view_name, view_offset in [
    ...
    bpy.context.scene.render.filepath = os.path.join(OUT_DIR, f"hair_v3_{view_name}.png")
    bpy.ops.render.render(write_still=True)
    print(f"  saved hair_v3_{view_name}.png")
